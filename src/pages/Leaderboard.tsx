    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500">Top performing students
            {selectedSubject && <span className="font-medium text-green-600"> · {subjects.find((s) => s.id === selectedSubject)?.name}</span>}
          </p>
        </div>
          ...
          {rankings.map((r, i) => (
            <div key={r.userId} className={`grid grid-cols-12 gap-4 p-4 border-b last:border-b-0 items-center hover:bg-gray-50 ${i < 3 ? "bg-yellow-50/50" : ""}`}>
              ...
              <div className="col-span-5">
                ...
                <div className="text-xs text-gray-400">{r.subject && r.subject !== "?" ? r.subject : r.email}</div>
              </div>
